...
To document the expected input format, we provide header files in `Headers/`. These files contain either the header row alone or the header row plus a few anonymised/example rows. Below we describe the main headers and corresponding units.
...
Header/example files in `Headers/Statistics RNA-Seq/` document the expected input structure for each R script. All header files are provided as comma-separated values (`.csv`) and include either only the header row or the header plus a small number of anonymised example rows.

* **`AgeDifferentialExpression_Matrisome_Genes_ByTissue_Headers.csv`**\
...
* **`TwoGroupDE_AllGenes_Parallelized_Headers.csv`**\
...
* **`EnsemblID_to_GeneSymbol_Mapping_biomaRt_Headers.csv`**\
...
* **`Limma_DifferentialExpression_BrainSites_SVA_SOM_data_Headers.csv`**\
  ...
  * Each row corresponds to a single gene.
* **`Limma_DifferentialExpression_BrainSites_SVA_SOM_pData_Headers.csv`**\
...
* **`PostHoc_BH_and_Fisher_CombinedPvalues_Headers.csv`**\
...
Quantitative measurements exported from ImageJ/Fiji are subsequently analysed in R; below we describe the expected structure of the corresponding tables:
...
Header/example files in `Headers/Graphs/` also define the structure of the tables used as direct numerical source data for the lollipop plots and heatmaps generated by the scripts in `code/`. Each file is a `.csv` containing the minimum set of columns needed to recreate the corresponding figure panel (plus, where appropriate, a few anonymised example rows).
...
* **`GTEx_Age_Lollipop_ByMatrisomeType_Headers.csv`**\
  ...
  * Each row corresponds to a single gene.
* **`GTEx_Age_Lollipop_Combined_MatrisomeAndSites_A_Headers.csv`**\
...
* **`GTEx_Age_Lollipop_Combined_MatrisomeAndSites_B_Headers.csv`**\
...
* **`ROBO_Pathway_Age_ZscoreHeatmap_Headers.csv`**\
...
* **`Code/Statistics RNA-Seq/DESeq2_Normalization_Brain_Paired5Sites.R`**\
...
* **`Code/Statistics RNA-Seq/DESeq2_Normalization_Brain_AgeByTissue_Unpaired.R`**\
...
* **`Code/Statistics RNA-Seq/AgeDifferentialExpression_Matrisome_Genes_ByTissue.R`**\
...
* **`Code/Statistics RNA-Seq/TwoGroupDE_AllGenes_Parallelized.R`**\
...
* **`Code/Statistics RNA-Seq/PostHoc_BH_and_Fisher_CombinedPvalues.R`**\
...
* **`Code/Statistics RNA-Seq/EnsemblID_to_GeneSymbol_Mapping_biomaRt.R`**\
...
* **`Code/Statistics RNA-Seq/Limma_DifferentialExpression_BrainSites_SVA_SOM.R`**\
...
* **`Code/Statistics IHQ/IHC_Background_Removal.ijm`**\
...
* **`Code/Statistics IHQ/ImageJ_BatchCrop_SaveROIs.ijm`**\
...
* **`Code/Statistics IHQ/ImageJ_ColorDeconvolution_BatchMeasurements.ijm`**\
...
* **`Code/Graphs/GTEx_Age_Lollipop_ByMatrisomeType.R`**\
...
* **`Code/Graphs/GTEx_Age_Lollipop_Combined_MatrisomeAndSites.R`**\
...
* **`Code/Graphs/Matrisome_Zscore_Heatmap_BrainSites.R`**\
...
* **`Code/Graphs/ROBO_Pathway_Age_ZscoreHeatmap.R`**\
...
* For **paired brain site analyses** (GTEx paired 5-site dataset):
  * Edit file paths at the top of `Code/Statistics RNA-Seq/DESeq2_Normalization_Brain_Paired5Sites.R` to point to your local count and metadata files.
...
* For **age-by-tissue analyses (unpaired)**:
  * Edit file paths in `Code/Statistics RNA-Seq/DESeq2_Normalization_Brain_AgeByTissue_Unpaired.R`.
...
* Edit file paths and sheet lists at the top of `Code/Statistics RNA-Seq/AgeDifferentialExpression_Matrisome_Genes_ByTissue.R` as needed.
* Run the script to obtain per-gene statistics by tissue and age group. The output tables can be saved in a user-defined folder (not included in this repository) and referenced as source data for the corresponding figures/tables.
...
* Adjust the group labels and sheet list at the top of `Code/Statistics RNA-Seq/TwoGroupDE_AllGenes_Parallelized.R`.
* Run the script to produce per-gene statistics, which can also be saved locally as figure/table source data.
...
* Edit file paths at the top of `Code/Statistics RNA-Seq/Limma_DifferentialExpression_BrainSites_SVA_SOM.R`.
...
* Save relevant outputs into a local folder of your choice (not version-controlled in this repository).

### 7. IHC image preprocessing and measurement

* **Background harmonisation (optional but recommended for pale/low-stain tissue):**
  * In Fiji, open `Code/Statistics IHQ/IHC_Background_Removal.ijm`.
...
* **Batch cropping to standard ROIs:**
  * Open `Code/Statistics IHQ/ImageJ_BatchCrop_SaveROIs.ijm` in Fiji.
...
* **Colour deconvolution and ROI measurements:**
  * Open `Code/Statistics IHQ/ImageJ_ColorDeconvolution_BatchMeasurements.ijm`.
...
* If you wish to aggregate multiple tests across sites or related contrasts:
  * Adapt `Code/Statistics RNA-Seq/PostHoc_BH_and_Fisher_CombinedPvalues.R` to your result tables.
...
* Whenever a results table contains Ensembl IDs but not gene symbols:
  * Use `Code/Statistics RNA-Seq/EnsemblID_to_GeneSymbol_Mapping_biomaRt.R` to add gene symbols via `biomaRt`.
  * Export annotated tables to be used in final figures/tables and in `figure_source_data/`.

### 10. Generate figure panels (RNA-seq)

* After running the normalisation and differential expression workflows described above, use the figure-generation scripts in `Code/Graphs/` (see “Figure generation scripts (RNA-seq)”):
...
This dataset contains de-identified data derived from human post-mortem brain tissue donors and from the Genotype-Tissue Expression (GTEx) project. All use of human biological material was approved by the Research Ethics Committee (CEP) of Hospital das Clínicas da Faculdade de Medicina da USP (HCFMUSP, CAPPesq), under Opinion number 5.803.114 and CAAE 52432220.5.0000.0068, as well as by the prior approvals that established the LIM15 brain biorepository. For the biorepository samples, the ethics committee granted a waiver of informed consent for secondary use of stored post-mortem material on the condition that only fully de-identified data would be shared. The GTEx component was used in accordance with GTEx/dbGaP data-use policies, and only de-identified expression data and non-identifying covariates are included in this Dryad submission.

All direct personal identifiers have been removed from the dataset prior to deposition. No names, initials, hospital or autopsy numbers, medical record numbers, dates of birth or death, addresses, contact details, or other institution-specific identifiers are included in the shared files. Each donor is represented only by a study-specific code. Demographic and clinical variables are restricted to non-identifying fields such as age (or age group), sex, brain region, broad cause-of-death categories, presence/absence of major comorbidities, post-mortem interval, and relevant technical variables. No free-text clinical notes or other potentially identifying narrative information are provided.

These de-identification procedures, together with the ethics approvals and consent waiver described above, ensure that the shared data comply with applicable legal and ethical guidelines and meet Dryad’s requirements for public release of human subjects data.

---

## Resubmission additions

The following six files supplement the original deposit and are provided at the repository's top level. Extract `analysis_inputs.zip` there to create `data/` before running the additional analyses.

| Added file | Purpose |
| --- | --- |
| `analysis_inputs.zip` | Ten analysis inputs under `data/`: local and GTEx expression tables, GTEx raw counts and cohort metadata, stored SLIT/ROBO age-test results, and the input for the original full-matrisome age analysis. These are analysis inputs, not header-only examples. |
| `GTEx_Age_Target_BH.R` | Applies Benjamini–Hochberg correction to 126 stored SLIT/ROBO age-test rows in `data/gtex_age_raw.xlsx`, without refitting tests. Reports BH7 within region/test, BH35 across five individual regions/test, and BH42 including pooled `All`. |
| `GTEx_Pathway_Age_Statistics.R` | Compares hippocampal expression between ages 60–79 and 20–59 using `data/gtex_pathway_age_expression.xlsx`. Reports Student, Welch and Mann–Whitney tests for all 45 pathway genes, with BH correction separately within each test. |
| `GTEx_Sex_DESeq2.R` | Fits within-region male-versus-female DESeq2 models to the paired or unpaired GTEx raw-count and metadata files. Reports all-gene adjusted P values separately from BH correction across the 20 target comparisons. |
| `GTEx_Sex_Welch.R` | Performs male-versus-female Welch comparisons for SLIT1–3 and ROBO2 across five regions, separately for the paired and unpaired GTEx expression tables, with BH correction across 20 comparisons per cohort. |
| `Local_Sex_Welch.py` | Performs female-versus-male Welch comparisons for SLIT1–3 and ROBO2 across eight local regions using `data/local_sex_expression.xlsx`, with BH correction across 32 planned comparisons. Non-estimable comparisons remain present with missing statistics. |

Command-line arguments are documented at the start of each script. Use separate, new output paths. The local script requires Python 3 with `numpy`, `pandas`, `openpyxl` and `scipy >=1.11`. The additional R scripts use `readxl`, `effsize`, `rstatix`, `coin`, `DESeq2` and `data.table` as applicable; the GTEx Welch script uses base R.
